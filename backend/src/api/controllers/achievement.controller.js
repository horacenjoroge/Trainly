...
const achievementController = {
  getUserAchievements: asyncHandler(async (req, res) => {
    const result = await achievementService.getUserAchievements(req.user.id, req.validated.query || {});
    res.json({ status: 'success', ...result });
  }),

  getLeaderboard: asyncHandler(async (req, res) => {
    const leaderboard = await achievementService.getLeaderboard(req.validated.query.limit || 50);
