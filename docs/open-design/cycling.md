...
<title>Trainly — Cycling Tracker</title>
<style>
...
.timer-wrap .paused-overlay{font-size:13px;font-weight:800;color:var(--warning);text-transform:uppercase;letter-spacing:.08em;margin-top:8px}
.map-card{margin:0 16px 16px;border:3px solid #333;background:#2A3A4C;height:160px;display:flex;align-items:center;justify-content:center;position:relative;overflow:hidden}
.map-card .map-text{font-size:13px;font-weight:600;color:#6B7280;text-align:center}
.map-card .map-text span{display:block;font-size:28px;margin-bottom:8px}
.map-card .map-line{position:absolute;top:20%;left:10%;width:80%;height:60%}
.map-card .map-line::before{content:'';position:absolute;top:30%;left:5%;width:90%;height:3px;background:var(--accent);transform:rotate(-10deg)}
.map-card .map-line::after{content:'';position:absolute;top:60%;left:10%;width:75%;height:3px;background:var(--accent);transform:rotate(5deg)}
...
.controls{display:flex;gap:12px;margin:0 16px;padding-bottom:16px;flex-wrap:wrap}
...
<div class="top-bar">
  <div class="title">
    <span style="font-size:20px">🚴</span> Cycling
...
<div class="stats-row">
  ...
  <div class="stat-box accent">
    <div class="sb-val">0.0 <span class="unit">km/h</span></div>
...
<div class="dialog-overlay" id="finishDialog">
  <div class="dialog">
    <h3>Save Workout?</h3>
    <p id="dialogSummary">0.0 km in 00:00 at 0.0 km/h. Save this ride to your history?</p>
...
<script>
let seconds=0,running=false,interval=null,distance=0;
...
function toggleRun(){
  const btn=document.getElementById('startBtn'),text=document.getElementById('startBtnText'),pausedLbl=document.getElementById('pausedLabel'),finishBtn=document.getElementById('finishBtn');
  if(!running){
    interval=setInterval(()=>{
      seconds++;document.getElementById('timerDisplay').textContent=formatTime(seconds);
      distance=((seconds/90)*0.8).toFixed(1);
      document.querySelectorAll('.stat-box .sb-val')[0].innerHTML=distance+' <span class="unit">km</span>';
      const speed=(distance/1)>0?(distance/(seconds/3600)).toFixed(1):'0.0';
      document.querySelectorAll('.stat-box.accent .sb-val')[0].innerHTML=speed+' <span class="unit">km/h</span>';
    },1000);
    running=true;btn.classList.remove('paused');text.textContent='⏸ Pause';pausedLbl.style.display='none';finishBtn.disabled=false;
  }else{
    clearInterval(interval);running=false;btn.classList.add('paused');text.textContent='▶ Resume';pausedLbl.style.display='block';
  }
}
function showFinishDialog(){
  clearInterval(interval);running=false;
  document.getElementById('startBtn').classList.add('paused');document.getElementById('startBtnText').textContent='▶ Resume';document.getElementById('pausedLabel').style.display='block';
  const speed=distance>0?(distance/(seconds/3600)).toFixed(1):'0.0';
  document.getElementById('dialogSummary').textContent=`${distance} km in ${formatTime(seconds)} at ${speed} km/h. Save this ride to your history?`;
...
function saveWorkout(){alert('Ride saved!');window.location='history.html'}
</script>
</body>
</html>
