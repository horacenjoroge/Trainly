...
class ApiService @Inject constructor(private val ac: ApiClient) {
    ...
    private suspend fun <T> call(api: suspend () -> retrofit2.Response<T>): NetworkResult<T> = try {
        val r = api()
        if (r.isSuccessful) r.body()?.let { NetworkResult.Success(it) } ?: NetworkResult.Error(ApiError("Empty body", r.code()))
        else NetworkResult.Error(ApiError(gson.fromJson(r.errorBody()?.string(), Map::class.java)?.get("message") as? String ?: "Error", r.code()))
    } catch (e: SocketTimeoutException) { NetworkResult.Error(ApiError.networkError("Timeout")) }
    catch (e: ConnectException) { NetworkResult.Error(ApiError.networkError("No connection")) }
    catch (e: IOException) { NetworkResult.Error(ApiError.networkError(e.message ?: "Network error")) }
    catch (e: Exception) { NetworkResult.Error(ApiError.unknownError(e)) }
