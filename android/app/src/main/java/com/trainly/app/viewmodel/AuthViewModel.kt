...
class AuthViewModel @Inject constructor(
    ...
    fun login() { val f=_s.value as? AuthUiState.LoginForm?:return; val err=validateLogin(f); if(err!=null){_s.value=f.copy(error=err);return}; _s.value=f.copy(isLoading=true); viewModelScope.launch { when(repo.login(f.email,f.password)){ is NetworkResult.Success -> _e.send(AuthFormEvent.LoginSuccess); is NetworkResult.Error -> _s.value=_s.value.let{ (it as AuthUiState.LoginForm).copy(isLoading=false, error=it.error) }; else -> {} } } }
    fun register() { val f=_s.value as? AuthUiState.RegisterForm?:return; val err=validateRegister(f); if(err!=null){_s.value=f.copy(error=err);return}; _s.value=f.copy(isLoading=true); viewModelScope.launch { when(repo.register(f.name,f.email,f.password)){ is NetworkResult.Success -> _e.send(AuthFormEvent.RegisterSuccess); is NetworkResult.Error -> _s.value=_s.value.let{ (it as AuthUiState.RegisterForm).copy(isLoading=false, error=it.error) }; else -> {} } } }
