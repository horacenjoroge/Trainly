...
class HomeViewModelTest {
    ...
    @BeforeEach fun setup() { Dispatchers.setMain(StandardTestDispatcher()); repo=mockk(); sm=mockk(relaxed=true); every{sm.authState}returns MutableStateFlow(AuthState(user=User("1","T","t@t",null,null,null,null), isAuthenticated=true, isLoading=false)) }
    @Test fun `success state after data loads`() = runTest { coEvery{repo.getPosts()}returns NetworkResult.Success(listOf(Post(id="1",userId="1",userName="T",comments=emptyList(),likes=emptyList(),createdAt=""))); coEvery{repo.getProgressStats()}returns NetworkResult.Success(ProgressStats()); vm=HomeViewModel(repo,sm); advanceUntilIdle(); assertTrue(vm.uiState.value is com.trainly.app.ui.designsystem.theme.UiState.Success) }
    @Test fun `error state when both fail`() = runTest { coEvery{repo.getPosts()}returns NetworkResult.Error(ApiError("n")); coEvery{repo.getProgressStats()}returns NetworkResult.Error(ApiError("n")); vm=HomeViewModel(repo,sm); advanceUntilIdle(); assertTrue(vm.uiState.value is com.trainly.app.ui.designsystem.theme.UiState.Error) }
}
