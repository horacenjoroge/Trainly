...
class TrackingViewModelTest {
    ...
    @Test fun `initialize resets stale state`() {
        vm.initialize("Running")
        vm.updateGpsPoint(GpsCoordinate(40.0,-74.0))
        vm.recordSplit()
        vm.updateDistanceMeters(1200.0)
        vm.initialize("Cycling")
        assertEquals("Cycling", vm.uiState.value.activityType)
        assertEquals(0, vm.uiState.value.splitCount)
        assertEquals(0, vm.uiState.value.gpsPoints.size)
        assertEquals(0.0, vm.uiState.value.distance)
    }
    ...
    @Test fun `manual distance update stores meters`() { vm.initialize("Swimming"); vm.updateDistanceMeters(250.0); assertEquals(250.0, vm.uiState.value.distance) }
